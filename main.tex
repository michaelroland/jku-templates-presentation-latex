...
\item Use option ``\verb|nightmode|'' to get a white-on-black theme designed for presentation in dimmed rooms.
\end{itemize}
\end{frame}
...
%\begin{table}
    ...
    \textverb{nojkulogo}           & Supress JKU \& K logos on title pages and in frame footers. \tabularnewline
    \textverb{noinstitutelogo}     & Use standard JKU logo even if \verb|\institutecode| is set.
                                     Can be toggled using \verb|\noinstitutelogo| and \verb|\endnoinstitutelogo| within the presentation. \tabularnewline
    \bottomrule
    \end{tabularx}
%\end{table}
\end{frame}
...
\item The starred version \verb|\maketitle*[<options>]| always uses the standard JKU logo instead of the current institute logo. See \string\institutecode\{code\} on slide~\ref{logo-commands}.
\end{itemize}
\end{frame}
...
\begin{frame}[containsverbatim,label={logo-commands}]
\frametitle{Logos}
...
Use \verb|\jkulogo[<mode options>]| to display a logo slide. That is a slide with only the JKU logo (or institute logo, if you defined one) centered on it. This slide is well suited as a final slide in your presentation.
...
\medskip
There is also a starred version \verb|\jkulogo*[<mode options>]|, which always uses the standard JKU logo instead of the current institute logo.
\end{frame}
...
Use \verb|\setcolormode[<mode options>]| to change the color mode (reset to default if no options are given).
\end{frame}

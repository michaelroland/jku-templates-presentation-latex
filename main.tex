...
%% 
%% _Important_: The document class should be the first like of LaTeX code in your main
...
%%  * appendixframenumber ... Similar to `totalframenumber', but total frame number only
%%                            counts frames in the main part before the `\appendix' command
%%                            and counts the overall total number of frames after that.
%% 
%% Sectioning options:
...
\begin{itemize}
\item Download the latest version of the theme package from \textbf{TODO}.
...
%\begin{table}
    ...
    \textverb{appendixframenumber} & Like ``\textverb{totalframenumber}'', but only count slides in the main part into the total. The overall total number of slides is only used after the \textverb{\string\appendix} command. \tabularnewline
    \bottomrule
    \end{tabularx}
%\end{table}
...
This a beamer theme for Johannes Kepler University Linz, Austria.

\bigskip
\begin{exampleblock}{Want to report a bug? Want to request improvements?}
You are always welcome to suggest improvements via the official repository at \textbf{TODO}.
\end{exampleblock}
\end{frame}

...
%%  * LIT  ... Use Linz Institute of Technology color scheme (automatically sets \institutecode to `LIT').
...
%% Command \copyrightnotice{text}: sets the copyright notice field
%\copyrightnotice{Copyright (c) \the\year\ \insertauthor. All rights reserved.}

%% Command \setwatermark[rotation in degrees]{large text}{small text}: sets the watermark shown on slides
%\setwatermark[30]{Space for your main watermark text.}{Space for your additional watermark text.}
...
\begin{frame}[label={options:colorscheme}]
...
\begin{frame}[label={options:colormode}]
...
\item Use option ``\textverb{darkmode}'' to display title, logo, and section frames with a dark background (primary color of the current color scheme).
\item Use option ``\textverb{nightmode}'' to get a white-on-black theme designed for presentation in dimmed rooms.
\end{itemize}
\end{frame}


\begin{frame}[label={options:slide-numbers}]
...
\smallskip
Btw., if you want to reference a slide number, use the option \textverb{[label=\{s\}]} in the target frame. Then \textverb{\string\ref\{s\}} will give a reference to the slide number. For instance, slide~\ref{options:section-slides} is the next slide.
\end{frame}


\begin{frame}[label={options:section-slides}]
\frametitle{Theme Options: Section Slides}
By default, each sectioning command (\textverb{\string\section}, \textverb{\string\subsection}, \textverb{\string\subsubsection}) inserts a section slide. You can globally disable section slides with:
...
%\begin{table}
    ...
    \textbf{Option}            & \textbf{Description} \tabularnewline
    \midrule
    \textverb{nosectionpage}       & Suppress all section slides. \tabularnewline
    \textverb{nosubsectionpage}    & Suppress subsection and subsubsection slides. \tabularnewline
    \textverb{nosubsubsectionpage} & Suppress subsubsection slides. \tabularnewline
    \bottomrule
    \end{tabularx}
%\end{table}
...
\smallskip
\begin{itemize}
\item The class option \textverb{handout} also suppresses these slides but includes them in slide numbering.
\item The starred versions of sectioning commands (e.g.\ \textverb{\string\section*\{...\}}) also suppress section slides. 
\end{itemize}
\end{frame}


\begin{frame}
\frametitle{Theme Options: Support for Historic Styles}
These options can be used to reproduce older and alternative versions of styles.
\smallskip
%\begin{table}
    \begin{tabularx}{\linewidth}{l>{\raggedright}X}
    \toprule
    \textbf{Option}			     & \textbf{Description} \tabularnewline
    \midrule
    \textverb{frametitlecaps}      & Set frame titles in capital letters. \tabularnewline
    \textverb{legacyitemizestyle}  & Use old bullet style in itemization. \tabularnewline
    \bottomrule
    \end{tabularx}
%\end{table}
\end{frame}
...
%\begin{table}
    ...
    \textbf{Option}			     & \textbf{Description} \tabularnewline
    \midrule
    \textverb{nofooter}            & Suppress slide footer. \tabularnewline
    \textverb{nojkufooter}         & Suppress JKU/partner logos in slide footer. \tabularnewline
    \textverb{noimprint}           & Suppress imprint on title slides. \tabularnewline
    \textverb{nojkulogo}           & Suppress JKU \& K logos on title pages and in frame footers. \tabularnewline
    \textverb{noinstitutelogo}     & Use standard JKU logo even if \textverb{\string\institutecode} is set.
                                     Can be toggled using \textverb{\string\noinstitutelogo} and \textverb{\string\endnoinstitutelogo} within the presentation. \tabularnewline
    \bottomrule
    \end{tabularx}
%\end{table}
\end{frame}
...
Use \textverb{\string\maketitle} to display a title slide. This command uses the following information:
...
%\begin{table}
    ...
    \textverb{\string\author\{...\}}    & The speakers / authors of the presentation.
                                          ...
                                          \tabularnewline
    \textverb{\string\institute\{...\}} & The institute / affiliations of the author(s). \tabularnewline
    \textverb{\string\date\{...\}}      & The date of the presentation. Defaults to today's date if omitted. Use \textverb{\string\date\{\}} to suppress the date. \tabularnewline
    \bottomrule
    \end{tabularx}
%\end{table}

\smallskip
\small An optional argument (\textverb{\string\cmd[opt]{...}}) allows to set short versions for use in e.g.\ footers.
\end{frame}


\begin{frame}[label={advanced-title-slides}]
\frametitle{Advanced Title Slides}

\begin{itemize}
\item You can change the presentation information and use the \textverb{\string\maketitle} command multiple times in your presentation.
\item An optional argument \textverb{\string\maketitle[<options>]} allows to change the color scheme of the title slide. Options may be one of:
    \begin{itemize}
    \item \textverb{light}: Use light color theme (or white-on-black in \textverb{nightmode}).
    \item \textverb{dark}: Use dark color theme.
    \item \textverb{gray}: Use JKU gray color theme.
    \item \textverb{black}: Use black background (or white background in \textverb{nightmode}).
    \end{itemize}
    In addition, options may contain a faculty name (see \hyperref[options:colorscheme]{Theme Options: Selecting a Color Scheme}) to use the faculty's color theme, e.g. ``\textverb{\string\maketitle[TNF,dark]}''.
\item The starred version \textverb{\string\maketitle*[<options>]} always uses the standard JKU logo instead of the current institute logo. See \textverb{\string\institutecode\{code\}} on slide~\ref{logo-commands}.
\end{itemize}
\end{frame}
...
\begin{frame}
\frametitle{Further Customizing Title Slides}
...
%\begin{table}
    ...
    \textbf{Command}                           & \textbf{Description} \tabularnewline
    \midrule
    \textverb{\string\series\{series title\}}  & A title printed in small above the presentation title (e.g.\ to mention the lecture name). \tabularnewline
    \textverb{\string\agenda[caption]\{text\}} & Can be used to add information next to the K logo on the next title slide. \tabularnewline
    \bottomrule
    \end{tabularx}
%\end{table}
\end{frame}
...
\begin{frame}[label={logo-commands}]
\frametitle{Logos}
...
\copyrightnotice{Space for your copyright notice.}
\begin{frame}
...
%\begin{table}
    ...
    \textbf{Command}                                 & \textbf{Description} \tabularnewline
    \midrule
    \textverb{\string\copyrightnotice\{...\}}        & Display a copyright notice on the left border of each slide. \tabularnewline
    \bottomrule
    \end{tabularx}
%\end{table}

\medskip
The copyright notice can be changed anywhere within a presentation by using \textverb{\string\copyrightnotice\{...\}} again.
Using \textverb{\string\copyrightnotice\{\}} removes the copyright notice from all following slides.
\end{frame}
\copyrightnotice{}
...
Use \verb|\setwatermark[<rotation>]{<large text>}{<small text>}| to display a watermark on each slide:

\begin{lstlisting}[language={[LaTeX]TeX},numbers=none]
\setwatermark[30]%
    {Space for your main watermark text.}%
    {Space for your additional watermark text.}
\end{lstlisting}

The optional argument ``\textverb{<rotation>}'' specifies the rotation angle in degrees.

The watermark can be changed anywhere within a presentation by using \verb|\setwatermark[...]{...}{...}| again.
Using \verb|\setwatermark{}{}| removes the watermark from all following slides.
\end{frame}
\setwatermark{}{}
...
A slides is called \textverb{frame} in \LaTeX\ beamer. It consists of a frame title and a body:
\begin{lstlisting}[language={[LaTeX]TeX},numbers=none]
\begin{frame}
...
Each column is wrapped into its own environment: \verb|\begin{column}{<width>}|.

You can set \textverb{<width>} e.g.\ to \verb|.45\textwidth| to let a column span 45\,\% of the normal text width.
\begin{lstlisting}[language={[LaTeX]TeX},numbers=none]
\begin{columns}[onlytextwidth,T]
\begin{column}{.45\textwidth}
Space for column 1.
...
Use the \textverb{\string\tableofcontents} command to list all sections and subsections in the presentation (if you really want to do this~\cite{schultz}):
...
The frame environment also takes optional arguments \verb|\begin{frame}[<options>]|. You already read about the ``\verb|label=|'' option on slide~\ref{options:slide-numbers}. Another option is \verb|plain| to suppress the footer for a frame.
...
\begingroup
\setbeamercolor{background canvas}{bg=jkuCyan}
\begin{frame}[containsverbatim,transparentfooter]
\frametitle{Transparent Footer}

Normally, the footer will hide any content that flows behind it. You can avoid this by making the footer transparent using the ``\textverb{transparentfooter}'' frame option.

\begin{lstlisting}[language={[LaTeX]TeX},numbers=none]
\begingroup
\setbeamercolor{background canvas}{bg=jkuCyan}
\begin{frame}[transparentfooter]
\frametitle{Space for your frame title.}
...
Use \textverb{\string\jkulogo[<mode options>]} to display a logo slide. That is a slide with only the JKU logo (or institute logo, if you defined one) centered on it. This slide is well suited as a final slide in your presentation.

Again, the optional argument ``\textverb{<mode options>}'' may be any of the color mode options that can be used for the \textverb{\string\maketitle} command (see slide~\ref{advanced-title-slides}), e.g.
\begin{columns}[onlytextwidth,T]
\column{.5\textwidth}
\begin{itemize}
...
\medskip
There is also a starred version \textverb{\string\jkulogo*[<mode options>]}, which always uses the standard JKU logo instead of the current institute logo.
\end{frame}
...
Just like switching the color mode and scheme for a specific special slide, you can also switch the color mode for the remaining part of the presentation. This may be useful if you want to create a presentation with parts focusing on more than one faculty. You can also use ``\textverb{night}'' and ``\textverb{nonight}'' to switch night mode on or off within the presentation.

Use \textverb{\string\setcolormode[<mode options>]} to change the color mode (reset to default if no options are given).
\end{frame}
...
\begin{frame}[containsverbatim,shrink=30]
\frametitle{Font Size and Slide Density}
Sometimes you may try to squeeze a lot of text or bullet items or code blocks onto a single slide ...
\begin{itemize}
\item and I mean a lot ...
\item really ... a lot ...
\item so much text that the slide eventually overflows.
\end{itemize}
Luckily, \LaTeX-beamer has a solution to your problem: The \verb|shrink| frame option. This option takes an optional shrink factor (percentage) as an argument, e.g. \verb|shrink=30|:
